...
void setup() {
  ...
  my_motor1 = new IBT2Motor(
        ...
        M1_DELAY_MS,
        M1_NO_JUMP_START, M1_NO_JUMP_START,
  ...
  d1_dance_moves[i++] =
    new DanceMove(
          ...
          EARTH_MOVE_UP,            // move_direction         motor direction l,r (l=down, r=up typically)
          EARTH_SPEED_UP,           // move_speed             pwm speed to run the motor at, 0-255
          EARTH_LONG_MOVE_DURATION, // move_duration_seconds  5 minutes, we just want to hit the limit to stop
  ...
  d1_dance_moves[i++] =
    new DanceMove(
          "m_down",
          0,                        // move_delay_seconds     seconds to delay before we start moving the motor
          EARTH_MOVE_DOWN,            // move_direction         motor direction l,r (l=down, r=up typically)
          EARTH_SPEED_UP,           // move_speed             pwm speed to run the motor at, 0-255
          EARTH_LONG_MOVE_DURATION, // move_duration_seconds  5 minutes, we just want to hit the limit to stop
          DELAY_TYPE_DANCE          // delay start DANCE 'd' or MOVE 'm'
          );
  ...
  my_dancer1 = new Dancer(
    ...
    EARTH_SPEED_DOWN,            // p_extend_speed speed to run the PMW at when extending
    EARTH_SPEED_DOWN             // p_center_speed   speed to run motor at when centering
    );
