#ifndef AAD_VANITYTABLE_H
#define AAD_VANITYTABLE_H
...
#include "AAE_vanitytable_config.h"
...
void setup() {
  ...
  m1_center_limit_switch = m1_bottom_limit_switch; // the bottom/left limit is the center/home
  ...
  d1_dance_moves[i++] =
    new DanceMove(
          ...
          VTBL_MOVE_UP,            // move_direction         motor direction l,r (l=down, r=up typically)
          VTBL_SPEED_UP,           // move_speed             pwm speed to run the motor at, 0-255
          VTBL_LONG_MOVE_DURATION, // move_duration_seconds  5 minutes, we just want to hit the limit to stop
  ...
  d1_dance_moves[i++] =
    new DanceMove(
          "m_down",
          0,                        // move_delay_seconds     seconds to delay before we start moving the motor
          VTBL_MOVE_DOWN,            // move_direction         motor direction l,r (l=down, r=up typically)
          VTBL_SPEED_UP,           // move_speed             pwm speed to run the motor at, 0-255
          VTBL_LONG_MOVE_DURATION, // move_duration_seconds  5 minutes, we just want to hit the limit to stop
          DELAY_TYPE_DANCE          // delay start DANCE 'd' or MOVE 'm'
          );
  ...
  my_dancer1 = new Dancer(
    ...
    VTBL_SPEED_DOWN,            // p_extend_speed speed to run the PMW at when extending
    VTBL_SPEED_DOWN             // p_center_speed   speed to run motor at when centering
    );
