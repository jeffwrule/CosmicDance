...
void Nucleon::new_dimm_cycle(unsigned long current_ms) {
    ...
    for (int i=0; i<NUM_QUARKS; i++){
      ...
      if (fibonocci_index+2 == NUM_FIBONOCCI) {
      ...
      quark_list[i].target_color = color_sequence[quark_list[i].color_index];
...
int Nucleon::adjust_color(int initial, int target) {
