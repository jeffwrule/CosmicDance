...
//#define Serial  SerialUSB //Serial for boards whith USB serial port
const int outputPin = 7;
const int zerocross = 2; // for boards with CHANGEBLE input pins
const int dimmer_delay_ms=500;  // time to delay between dim operations
const int dimmer_pause_ms=1000;  // time to delay between dim operations
...
void loop() 
{
  ...
  for (int i=0; i <= 100; i++) {
    ...
    Serial.println(i);
    delay(dimmer_delay_ms);
  }

  delay(dimmer_pause_ms);
