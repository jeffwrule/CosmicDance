...
class ZeroCrossDimmer {
 ...
 private:
    ...
    volatile uint8_t pin_list[NUM_DIMMERS];       // list of pin to dimmer mappings
    volatile int16_t *levels[NUM_DIMMERS];        // pointer to the current dimm level(s) inside each Light struct for each pin
...
//                          uint8_t dimmer_number);   // sets the individual dimmer to the given value dimmer_numbers is >= 0 and < num_dimmers
    ...
    void blink(uint8_t num_blinks=1); // blink all the lights off and on
    ...
    ZeroCrossDimmer(Light **light_list, 
      ...
      triac_lock_delay_us(triac_lock_delay_us) {
        Serial.println(F("ZeroCrossDimmer Constructor Starting"));
        ...
        for (uint8_t i=0; i<num_dimmers; i++) {
          pin_list[i] = light_list[i]->pin;
          levels[i] =  &(light_list[i]->level);  // a pointer to the level in the lights array; changes to lights will be reflected in this array too
        ...
        enableInterrupt(zero_cross_pin, zero_cross_isr, RISING);
...
void ZeroCrossDimmer::blink(uint8_t num_blinks=1) {

  int16_t old[NUM_DIMMERS];
  int16_t light_off=DIMM_LEVEL_OFF;
  int16_t light_on=DIMM_LEVEL_ON;

  // save the old dimming values
  for (uint8_t i=0; i<NUM_DIMMERS; i++ ) {
    old[i] = *levels[i];
  }

  for (uint8_t i=0; i<num_blinks ; i++ ) {
    for(uint8_t j=0; j<NUM_DIMMERS; j++) { *levels[j] = light_on; }
    delay(500);   // display on  shortly
    for(uint8_t j=0; j<NUM_DIMMERS; j++) {*levels[j] = light_off; }
    delay(500);   // display that shortly
  }
...
void ZeroCrossDimmer::display() {
  Serial.println(F("ZeroCrossDimmer::display() List: "));
