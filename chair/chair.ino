...
unsigned long rock_interval_started;     // time when we last flipped the rocking on or off
unsigned long rock_interval_length;      // time to wait for the next rock interval
#define  ROCK_ON_INTERVAL 750            // rocking seems to work best when we are on slightly longer then off
...
void rock_chair() {
  ...
  if (cur_time < rock_interval_started) {
    rock_interval_started = cur_time;
    return;
  }
  ...
  if (is_dancing) {
    ...
    if (is_rocking) {
      cur_diff = cur_time - rock_interval_started;
      // stop/start individual rocks
      if (cur_diff >= rock_interval_length) {
        rock_interval_started = cur_time;
        if (rock_interval_active) {
          digitalWrite(v12Switch, LOW);
          rock_interval_length = ROCK_OFF_INTERVAL;
          rock_interval_active = false;
    ...
    cur_diff  = cur_time - rocking_started;
