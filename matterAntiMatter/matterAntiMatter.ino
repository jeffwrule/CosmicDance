...
unsigned long num_increments = 200;
...
//unsigned int fibonicci_sequence[NUM_FIBONOCCI] = {  8000, 5000, 3000, 2000, 1000, 1000};
...
struct light_box { 
    ...
    int increment_r;         // increment for the red channel
    int increment_g;         // incrmeent for the green channel
    int increment_b;         // increment for the blue channel
    };
...
unsigned long get_avg_update_time() { 
  ...
  return (avg_update_time.total_ms / avg_update_time.count) < 3 ? 3 : avg_update_time.total_ms / avg_update_time.count ; 
}
...
void new_dimm_cycle(unsigned long current_ms) {
    ...
    if (num_increments > MAX_GRAY) num_increments = MAX_GRAY; // can't have more resulution then our LEDs
...
void status(char *msg) {
  ...
  Serial.print("Source:"); Serial.println(msg);
  ...
  Serial.print(F("Update Avarages: iteration count="));
  Serial.print(avg_update_time.count);
  Serial.print(F(", iteration total_ms="));
  Serial.print(avg_update_time.total_ms);
  Serial.print(F(", iteration avg="));
  Serial.println(avg_update_time.total_ms / avg_update_time.count);
  Serial.println(F(""));
...
//    color current_color;              // current color level
//    color target_color;               // color we are headed for
//    unsigned int increment_r;         // increment for the red channel
//    unsigned int increment_g;         // incrmeent for the green channel
//    unsigned int increment_b;         // increment for the blue channel
  ...
  // moving average info
  Serial.print(F("Moving Average avg_ms="));
...
void loop() {
  ...
  if (dimm_cycle.to_white_complete &&  dimm_cycle.to_color_complete ) {
    ...
    Serial.println(F("DIMM_TO_COLOR COMPLETE, NEW CYCLE"));
    status("Everything Complete");
  }

//  // did we finish the first part of the dimm
//  if (dimm_cycle.to_white_complete && !dimm_cycle.to_color_complete ) {
//    reverse_dimm_cycle();
//    Serial.println(F("DIMM_TO_WHITE COMPLETE"));
//    status();
//  }
  ...
  if (current_ms - dimm_cycle.ms_last_step >= dimm_cycle.ms_per_step) {
    ...
    for (int i=0; i<NUM_BOXES; i++) {
     ...
     // lets see on average how long it takes to run a dimm step
