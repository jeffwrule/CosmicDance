...
#define IS_BRIEF False

#define LIMITSWITCHES      // changes the code to use the A1 pin for a LMIT switch rather then ON/OFF
...
const int LeftLimit = A2;                 // Left will also be down limit
const int RightLimit = A1;                // Right will also be up
...
void setup() {
  ...
  pinMode(RxD, INPUT);
  pinMode(TxD, OUTPUT);
#ifdef LIMITSWITCHES      // conditionally add support for limit switches, drops support v5Switch
...
void loop() {
  ...
  check_for_fob();  // is the fob pushed?
